...
def test_page_front_matter():
    ...
    for page in pages:
        with open(page, 'r') as f:
            ...
            # SEO checks
            assert len(front_matter['description']) > 50, f"Page {page} description too short for SEO"
            assert len(front_matter['description']) <= 160, f"Page {page} description too long for SEO"
...
def test_structured_data_valid():
    ...
    with open(seo_path, 'r') as f:
        ...
        json_ld_regex = re.compile(r'<script type="application/ld\+json">(.*?)</script>', re.DOTALL)
    ...
    for page in pages:
        with open(page, 'r') as f:
            ...
            if "nav-buttons" in content or "→" in content:
                ...
                assert len(links) > 0 or "github.com" in content, f"Page {page} should have proper internal links"
...
def test_keywords_in_pages():
    ...
    for page in pages:
        with open(page, 'r') as f:
            ...
            keyword_count = sum(1 for keyword in important_keywords if keyword.lower() in content)
